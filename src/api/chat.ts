...
Messages.map((item: any) => {
  let tmp = User().find((x: any) => x.uuid === item.userId) as any;
  item.user = {
    uuid: tmp.uuid,
    name: tmp.name,
    avatar: tmp.avatar
...
Groups.map((item: any) => {
  item.messages = Messages.filter((x: any) => x.chatId === item.uuid) as any;
  item.user = User().find((x: any) => x.uuid === item.created_by) as any;

  return item;
});

// get chat group
const getChatById = (uuid: string) => {
