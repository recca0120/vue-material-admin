...
// add user to mail
MailItem.map((item: any) => {
  let users = User.getUser() as any;
  item.from = users.find((x: any) => x.uuid === item.fromId);
...
const getMail = (limit: number | null = null) => {
  return limit ? MailItem.slice(0, limit) : MailItem;
};

const getMailById = (uuid: string | undefined = undefined) => {
  return uuid === undefined ? MailItem[0] : MailItem.find(x => x.uuid === uuid);
};

const getMailByType = (type: string) => {
